...
class InvoiceEmailService {
  ...
  Future<void> processAndSendInvoice({
    ...
    try {
      ...
      if (!docSnap.exists) {
        debugPrint('InvoiceEmailService: payment_logs not found for $txnId');
        return;
      }

      final data = docSnap.data()!;
      if (data['invoiceSent'] == true) {
        debugPrint('InvoiceEmailService: Invoice already sent for $txnId');
        return;
      }

      // Generate invoice number if not exists
      final invoiceNumber = data['invoiceNumber'] ?? 'INV-${txnId.substring(0, 8).toUpperCase()}';
      ...
      final paymentDate = (data['timestamp'] as Timestamp?)?.toDate() ?? DateTime.now();
      ...
      await _db.collection('mail').add({
        'to': customerEmail,
        'message': {
          'subject': 'Your Invoice from Rooks Brooks IT Solutions - $invoiceNumber',
      ...
      debugPrint('InvoiceEmailService: Successfully queued email for \$txnId');

    } catch (e) {
      debugPrint('InvoiceEmailService: Error sending invoice for \$txnId: \$e');
  ...
  Future<void> retryPendingInvoices() async {
    try {
      final snapshot = await _db.collection('payment_logs')
      ...
      debugPrint('InvoiceEmailService: Found \${snapshot.docs.length} pending invoices to retry');
      ...
      for (var doc in snapshot.docs) {
        ...
        if (data['status'] == 'SUCCESS' && data['registrationCompleted'] == true) {
          ...
          if (uid != null && uid.toString().isNotEmpty) {
             final userQuery = await _db.collectionGroup('users').where(FieldPath.documentId, isEqualTo: uid).limit(1).get();
